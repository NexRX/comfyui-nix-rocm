...
- Library Paths: Automatically includes `/run/opengl-driver/lib` on Linux for NVIDIA drivers and ROCm library paths for AMD GPUs
...
ROCm support has been fully integrated into the project, providing AMD GPU acceleration for ComfyUI.

### Key Features
- **Pre-built PyTorch Wheels**: Uses PyTorch 2.10.0 with ROCm 7.1 from pytorch.org (~2GB download)
- **Supported GPUs**: AMD Radeon RX 6000 and 7000 series
- **Bundled Libraries**: ROCm 7.1 runtime included in wheels (no separate ROCm toolkit needed)
- **Fast Builds**: No compilation required, just wheel downloads

### Usage
- **Run with ROCm**: `nix run .#rocm`
- **Build ROCm package**: `nix build .#rocm`
- **Build ROCm Docker**: `nix run .#buildDockerRocm`
- **Run ROCm Docker**: `docker run --device=/dev/kfd --device=/dev/dri -p 8188:8188 -v $PWD/data:/data comfy-ui:rocm`
...
### Files Modified
- `nix/versions.nix`: Added ROCm wheel URLs (torch 2.10.0, torchvision 0.25.0, torchaudio 2.10.0)
- `nix/python-overrides.nix`: Added ROCm torch/torchvision/torchaudio packages with ROCm libraries
- `nix/packages.nix`: Added ROCm library paths and useRocm flag
...
- HSA environment variables set for ROCm runtime compatibility
